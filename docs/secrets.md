...
- **Azure AI Search** (nextjs + mcp-gateway): AKS creates the service by default (`create_azure_aisearch`) and defaults **`RAG_VECTOR_STORE=azureaisearch`**. On GKE/EKS, pass BYO `azure_aisearch_endpoint` / `azure_aisearch_key` (and set `rag_vector_store = "azureaisearch"`). Index `vectorsearch` is created by the app on first embed. Optional `azure_aisearch_index`, `azure_aisearch_query_type` (default `similarity_hybrid`). Set `rag_vector_store` to `pinecone` or `singlestore` on AKS only if you are not using Search.
- **Azure OpenAI** (`AZURE_OPENAI_*`; nextjs + backend + mcp-gateway): KB embed runs on the app, agents on the backend, and query-time RAG on mcp-gateway. AKS creates the account by default (`create_azure_ai`). On GKE/EKS, pass BYO `azure_openai_api_key` (+ instance/version/base path). Terraform does not deploy chat or embedding models — they do that in Azure. Default AKS `azure_ai_public_network_access = false` (private endpoints). Empty `azure_ai_location` uses the AKS region.
- **Anthropic** (`ANTHROPIC_API_KEY`; nextjs + backend + mcp-gateway): set `anthropic_api_key`. Platform Claude agents (including workflows.builder) run in Next.js and read this from `nextjs-secrets`.
