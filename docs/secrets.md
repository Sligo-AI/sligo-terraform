...
- **Azure AI Search** (nextjs + mcp-gateway): AKS creates the service by default (`create_azure_aisearch`). On GKE/EKS, pass BYO `azure_aisearch_endpoint` / `azure_aisearch_key`. Index `vectorsearch` is created by the app on first embed. Optional `azure_aisearch_index`, `azure_aisearch_query_type` (default `similarity_hybrid`).
