...
- **Nginx Ingress Controller** (LoadBalancer). Helm sets `proxy-body-size: 110m` and a 1800s read timeout so knowledge uploads (up to 100 MB) and long agent replies can pass through the app.
...
By default **`create_azure_ai`** and **`create_azure_aisearch`** create an Azure OpenAI-compatible account and Azure AI Search in the AKS region (no model deployments, no Search index). Access is **private** (VNet endpoints, same pattern as Redis). `AZURE_OPENAI_*` is injected into **nextjs, backend, and mcp-gateway** secrets; `AZURE_AISEARCH_*` into **nextjs and mcp-gateway**. AKS also defaults **`RAG_VECTOR_STORE=azureaisearch`** so knowledge-base embeddings use Search rather than Pinecone. Set `rag_vector_store` to `pinecone` or `singlestore` to override. Set `create_azure_ai` / `create_azure_aisearch` to `false` and pass BYO `azure_openai_*` / `azure_aisearch_*` if they already have the services. See [secrets.md](../secrets/#optional-azure-ai-search-azure-openai-and-amazon-bedrock).
